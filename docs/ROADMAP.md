...
| Public Mount URL plumbing | ✅ | `proxyToSandbox` gate + `exposePort(8000, {hostname, token: streamToken(sid)})` (stable token → predictable URL, re-activated each ride/remount) + `PUBLIC_DOMAIN` var + wildcard DNS `*.<PUBLIC_DOMAIN>` → Worker. **Deploy constraint:** `PUBLIC_DOMAIN` must be a *dedicated* zone apex you own (a root domain) — a shared-apex wildcard shadows sibling workers (real incident) and isn't free-SSL-isolatable without Enterprise sub-zones; see README caveats. `defaultPort(3000≠8000)` is why `exposePort` pins port 8000. `streamUrl` is persisted to the registry and returned by `ride`/`session`/`remount` + `/status`. Optional follow-ups: `keepAlive` for active streams; `unexposePort` on `retire`. |
